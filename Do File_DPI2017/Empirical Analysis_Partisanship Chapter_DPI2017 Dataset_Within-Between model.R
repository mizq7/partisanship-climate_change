...
*generate mean variables
	...
	egen execrlc_mean = mean(execrlc), by(c_code)
...
*generate within variables
	...
	egen execrlc_mean_new = mean(execrlc), by(c_code)
	...
	gen execrlcw = execrlc - execrlc_mean_new
...
*center variables
	...
	sum execrlc, meanonly
	gen cexecrlc = execrlc - r(mean)
	...
	sum execrlc_mean, meanonly
	gen cexecrlc_mean = execrlc_mean - r(mean)
...
* Preparation for the within-between estimation
	...
	order co2pc year execrlc lngdppc trade pop_wdi urban renew forest annexI///
...
* residuals check for the final model:
	...
	xtreg co2pc cons execrlcw lngdppcw tradew yearw popw urbanw reneww///
	forestw annexIw islandw latitudew i.year execrlc_mean clngdppc_mean///
...
*Final model:
	...
	reg co2pc cons execrlcw lngdppcw tradew yearw popw urbanw reneww forestw///
	annexIw islandw latitudew i.year execrlc_mean clngdppc_mean ctrade_mean///
...
*Null Model
	...
	runmlwin co2pc cons if countryname!="Qatar", level2(c_code: cons)///
	level1(year: cons) nopause rigls

	estimates store Mod1_REnull

	outreg2 using output_co2ideo1, dec(3) excel label///
	alpha (0.001, 0.01, 0.05, 0.1) symbol(***, **, *, ^)///
	ctitle(Model 1:Null) drop(i.year) replace

*Fixed Effect Model

	xtreg co2pc execrlc clngdppc ctrade cyear i.year if countryname!="Qatar", fe
...
*Random Effect Model
	...
	runmlwin co2pc cons execrlc clngdppc ctrade cyear i.year if///
...
*Random Effect Within-Between Model
	...
	runmlwin co2pc cons execrlcw lngdppcw tradew yearw execrlc_mean///
	clngdppc_mean ctrade_mean i.year if countryname!="Qatar",///
...
*Fixed Effect Model
	...
	xtreg co2pc execrlc clngdppc ctrade cyear cpop curban crenew cforest///
	cannexI cisland clatitude i.year if countryname!="Qatar", fe

	estimates store Mod2_FE

	outreg2 using output_co2ideo2, dec(3) excel label///
	alpha(0.001, 0.01, 0.05, 0.1) symbol(***, **, *, ^)///
	addtext(Year FE, Yes) ctitle(Model 2:FE) drop(i.year) append

*Random Effect Model

	runmlwin co2pc cons execrlc clngdppc ctrade cyear cpop curban crenew///
	cforest cannexI cisland clatitude i.year if countryname!="Qatar",///
	level2(c_code: cons) level1(year: cons) nopause rigls
...
**Random Effect Within-Between Model
	...
	runmlwin co2pc cons execrlcw lngdppcw tradew yearw popw urbanw reneww///
	forestw annexIw islandw latitudew i.year execrlc_mean clngdppc_mean///
	ctrade_mean cpop_mean curban_mean crenew_mean cforest_mean cannexI_mean///
	cisland_mean clatitude_mean if countryname!="Qatar", level2(c_code: cons)///
	level1(year: cons) nopause rigls
