...
xtreg co2pc cpdleft_agg lngdppc trade pop_wdi urban renew forest annexI island latitude, re vce (cluster c_code)
...
xtline co2pc if cpdleft_agg>=200, overlay
...
reg co2pc cpdleft_agg lngdppc trade, robust
outreg2 using output_co2cpd1, dec(3) excel label alpha(0.001, 0.01, 0.05, 0.1) symbol(***, **, *, ^) addtext(Year FE, No) ctitle(Model 1: OLS) drop(i.year) replace

*Random-effects GLS regression

xtreg co2pc cpdleft_agg lngdppc trade i.year, vce(cluster c_code)
outreg2 using output_co2cpd1, dec(3) excel label alpha(0.001, 0.01, 0.05, 0.1) symbol(***, **, *, ^) addtext(Year FE, Yes) ctitle(Model 1: GLS, Random) drop(i.year) append

*Panel corrrected standard errors (XTPCSE)

xtpcse co2pc cpdleft_agg lngdppc trade i.year, corr(ar1) pairwise
outreg2 using output_co2cpd1, dec(3) excel label alpha(0.001, 0.01, 0.05, 0.1) symbol(***, **, *, ^) ctitle(Model 1: PCSE) drop(i.year) append

*GLS random-effects regression with Driscoll and Kraay standard errors (XTSCC)

xtscc co2pc cpdleft_agg lngdppc trade i.year, lag(4)
outreg2 using output_co2cpd1, dec(3) excel label alpha(0.001, 0.01, 0.05, 0.1) symbol(***, **, *, ^) addtext(Year FE, Yes) ctitle(Model 1: XTSCC) drop(i.year) append

* MODEL 2

*Pooled OLS with Heteroskedasticity Robust Standard Error

reg co2pc cpdleft_agg lngdppc trade pop_wdi urban renew forest annexI island latitude, robust
outreg2 using output_co2cpd2, dec(3) excel label alpha(0.001, 0.01, 0.05, 0.1) symbol(***, **, *, ^) addtext(Year FE, No) ctitle(Model 2: OLS) drop(i.year) replace

*Random-effects GLS regression

xtreg co2pc cpdleft_agg lngdppc trade pop_wdi urban renew forest annexI island latitude i.year, vce(cluster c_code)
outreg2 using output_co2cpd2, dec(3) excel label alpha(0.001, 0.01, 0.05, 0.1) symbol(***, **, *, ^) addtext(Year FE, Yes) ctitle(Model 2: GLS, Random) drop(i.year) append


*Panel corrrected standard errors (XTPCSE)

xtpcse co2pc cpdleft_agg lngdppc trade pop_wdi urban renew forest annexI island latitude i.year, corr(ar1) pairwise
outreg2 using output_co2cpd2, dec(3) excel label alpha(0.001, 0.01, 0.05, 0.1) symbol(***, **, *, ^) addtext(Year FE, Yes) ctitle(Model 2: PCSE) drop(i.year) append

*GLS random-effects regression with Driscoll and Kraay standard errors (XTSCC)

xtscc co2pc cpdleft_agg lngdppc trade pop_wdi urban renew forest annexI island latitude i.year, lag(4)
outreg2 using output_co2cpd2, dec(3) excel label alpha(0.001, 0.01, 0.05, 0.1) symbol(***, **, *, ^) addtext(Year FE, Yes) ctitle(Model 2: XTSCC) drop(i.year) append
...
xtreg co2pc fed_lij lngdppc trade i.year if countryname!="Qatar", vce(cluster country)
...
xtscc co2pc fed_lij lngdppc trade i.year if countryname!="Qatar", re lag(4)
outreg2 using output_co2fed, dec(3) excel label alpha(0.001, 0.01, 0.05, 0.1) symbol(***, **, *, ^) addtext(Year FE, Yes) ctitle(Model 1) drop(i.year) append
...
xtreg co2pc fed_lij lngdppc trade pop urban renew forest annexI island latitude i.year if countryname!="Qatar", vce(cluster country)
outreg2 using output_co2fed, dec(3) excel label alpha(0.001, 0.01, 0.05, 0.1) symbol(***, **, *, ^) addtext(Year FE, Yes) ctitle(Model 1) drop(i.year) append
...
xtscc co2pc fed_lij lngdppc trade pop urban renew forest annexI island latitude i.year if countryname!="Qatar", re lag(4)
outreg2 using output_co2fed, dec(3) excel label alpha(0.001, 0.01, 0.05, 0.1) symbol(***, **, *, ^ ) addtext(Year FE, Yes) ctitle(Model 1) drop(i.year) append
